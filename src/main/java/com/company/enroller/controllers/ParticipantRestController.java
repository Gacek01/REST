...
public class ParticipantRestController {
	...
	// localhost:8080/participants/user3
	...
	// http://localhost:8080/participants/user1
	@RequestMapping(value = "", method = RequestMethod.POST)
	public ResponseEntity<?> registerParticipant(@RequestBody Participant participant) {
		if (participantService.findByLogin(participant.getLogin()) != null) {
			return new ResponseEntity<String>("Unable to add - participant already exists", HttpStatus.CONFLICT);
		}
	...
	// http://localhost:8080/participants/user2
	@RequestMapping(value = "/{id}", method = RequestMethod.DELETE)
	public ResponseEntity<?> deleteParticipant(@PathVariable("id") String login) {
		Participant participant = participantService.findByLogin(login);
		if (participant == null) {
			return new ResponseEntity<String>("Unable to delete - participant does not exist", HttpStatus.NOT_FOUND);
		}
		participantService.delete(participant);
		return new ResponseEntity<Participant>(HttpStatus.NO_CONTENT);
	}

	// PUT update specific participant
	// http://localhost:8080/participants/user2
